...
public class ConvertTreeForPerformance {
	
	public static final String start = "start";
	public static final String complete = "complete";

	public static Pair<ProcessTree, Map<UnfoldedNode, UnfoldedNode>> convert(ProcessTree oldTree) {
		ProcessTree newTree = new ProcessTreeImpl();
		Map<UnfoldedNode, UnfoldedNode> mapping = new THashMap<>();
		newTree.setRoot(convert(new UnfoldedNode(oldTree.getRoot()), newTree, mapping, null));
		return Pair.of(newTree, mapping);
	}

	private static Node convert(UnfoldedNode unode, ProcessTree newTree, Map<UnfoldedNode, UnfoldedNode> mapping, UnfoldedNode newParent) {
		if (unode.getNode() instanceof Block) {
			...
			for (Node child : unode.getBlock().getChildren()) {
				newNode.addChild(convert(unode.unfoldChild(child), newTree, mapping, childParent));
			}
			...
			Manual aStart = new org.processmining.processtree.impl.AbstractTask.Manual(unode.getNode().getName() + "+" + start);
			aStart.setProcessTree(newTree);
			newTree.addNode(aStart);
			xor.addChild(aStart);
			mapping.put(childParent.unfoldChild(xor).unfoldChild(aStart), unode);
			
			Manual aComplete = new org.processmining.processtree.impl.AbstractTask.Manual(unode.getNode().getName() + "+" + complete);
