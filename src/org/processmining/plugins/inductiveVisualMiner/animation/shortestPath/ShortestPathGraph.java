...
public class ShortestPathGraph {
	...
	private DefaultDirectedGraph<ShortestPathNode, ShortestPathEdge> graph = new DefaultDirectedGraph<>(
	...
	public List<LocalDotEdge> getShortestPath(LocalDotNode from, LocalDotNode to) {
		List<ShortestPathEdge> path;
		try {
			path = DijkstraShortestPath.findPathBetween(graph, node2end.get(from),
					node2begin.get(to));
